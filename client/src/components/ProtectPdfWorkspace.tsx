...
import { UploadCloud, X, Download, FileText, ShieldCheck, Eye, EyeOff, Loader2 } from 'lucide-react';
...
export default function ProtectPdfWorkspace({
  ...
  const handleProtect = async () => {
    ...
    if (!password.trim()) {
      showToast('Please specify a password to protect the PDF.', 'error');
      return;
    }

    setIsProcessing(true);

    // 1. Create a dummy queue item using the store
    const item = {
      file,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            ...
            <div className="flex flex-col gap-4 bg-white/[0.02] border border-white/10 rounded-2xl p-5 sm:p-6 shadow-xl">
              <div>
                ...
                <p className="text-xs text-white/40 font-light">Set a secure password for your PDF</p>
              </div>
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl p-5 sm:p-6 flex flex-col gap-4">
                  ...
                  {(currentItem.status === 'uploading' || currentItem.status === 'converting') && (
                    <div className="flex flex-col gap-2">
                      <div className="flex justify-between text-xs text-white/60">
                        <span>
                          {currentItem.status === 'uploading' ? 'Uploading PDF...' : 'Securing PDF...'}
                        </span>
                  ...
                  {currentItem.status === 'done' && currentItem.downloadUrl && (
                    <div className="flex flex-col gap-4">
                      <div className="flex items-center gap-2 text-white/80 text-xs">
                        ...
                        <span>PDF is now encrypted & password-protected!</span>
                      </div>
                      
                      <a
                        href={`${import.meta.env.VITE_API_URL || ''}${currentItem.downloadUrl}`}
                        download={currentItem.outputFilename || 'protected.pdf'}
                  ...
                  {currentItem.status === 'error' && (
                    <div className="text-xs text-red-400 font-medium bg-red-500/10 border border-red-500/20 p-3 rounded-lg flex items-center gap-2">
                      ...
                      <span>{currentItem.error || 'Failed to protect PDF.'}</span>
                    </div>
                  )}
                </div>
              )}
